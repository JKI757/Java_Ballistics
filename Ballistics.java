 ...
 *
 * @author josh
 * This is as pure of a port from Derek Yate's GNU Ballistics library into Java as I can manage
 * It is released under the GPL, as the original was.
 */


public class Ballistics {

	private static final int BCOMP_MAXRANGE = 50001;
	
	private static final double GRAVITY = -32.194;
	
	private static double DegtoMOA(final double deg){
		return deg*60.0;
	}
	...
	private static double MOAtoDeg(double moa){
		return moa/60.0;
	}
	private static double MOAtoRad(double moa){
		return moa/60.0*Math.PI/180.0;
	}
	private static double RadtoDeg(double rad){
		return rad*180.0/Math.PI;
	}
	private static double RadtoMOA(double rad){
	...
	private static double AtmCorrect(double DragCoefficient, double Altitude, double Barometer, double Temperature, double RelativeHumidity){
	...
	private static double retard(int DragFunction, double DragCoefficient, double Velocity){
...
//		printf("DF: %d, CD: %f, V: %f,);
	...
	private static double Windage(double WindSpeed, double Vi, double xx, double t){
	...
	private static double HeadWind(double WindSpeed, double WindAngle){
	...
	private static double CrossWind(double WindSpeed, double WindAngle){
		double Wangle = DegtoRad(WindAngle);
	...
	public static double ZeroAngle(int DragFunction, double DragCoefficient, double Vi, double SightHeight, double ZeroRange, double yIntercept){
		...
		do {
			...
			//if  quit=1; // If our accuracy is sufficient, we can stop approximating.
			//if  quit=1; // If we exceed the 45 degree launch angle, then the projectile just won't get there, so we stop trying.

		} 
                while ( !((Math.abs(da) < MOAtoRad(0.01)) || (angle > DegtoRad(45))));
	...
	public int SolveAll(int DragFunction, double DragCoefficient, double Vi, double SightHeight, double ShootingAngle, double ZAngle, double WindSpeed, double WindAngle, double[] Solution){
		...
		//final int size = 10*BCOMP_MAXRANGE + 2;
		//double[] ptr;
		//ptr = new double[size];
		...
		do {
			...
			if (x/3>=n){
				Solution[10*n+0]=x/3;							// Range in yds
				Solution[10*n+1]=y*12;							// Path in inches
				Solution[10*n+2]=-RadtoMOA(Math.atan(y/x));			// Correction in MOA
				Solution[10*n+3]=t+dt;							// Time in s
				Solution[10*n+4]=Windage(crosswind,Vi,x,t+dt); 	// Windage in inches
				Solution[10*n+5]=RadtoMOA(Math.atan((Solution[10*n+4]/12)/(Solution[10*n+0]*3)));	// Windage in MOA
				Solution[10*n+6]=v;								// Velocity (combined)
				Solution[10*n+7]=vx;							// Velocity (x)
				Solution[10*n+8]=vy;							// Velocity (y)
				Solution[10*n+9]=0;								// Reserved
				n++;	
			}	
		...
		}while ( !((Math.abs(vy)>Math.abs(3*vx)) || (n>=BCOMP_MAXRANGE+1)) );

		Solution[10*BCOMP_MAXRANGE+1]=(double)n;
	...
	private static final int TOO_HIGH = 2;
	private static final int PBR_ERROR = 3;
	...
	public int pbr(int DragFunction, double DragCoefficient, double Vi, double SightHeight, double VitalSize, int[] oresult){
		...
		do {
				...
				Gy=GRAVITY*Math.cos(DegtoRad((ShootingAngle + ZAngle)));
				Gx=GRAVITY*Math.sin(DegtoRad((ShootingAngle + ZAngle)));
				...
				do {
					...
					if (n>=BCOMP_MAXRANGE+1) { result=PBR_ERROR; break;}
				...
				} while ( !(keep==1 && keep2==1 && min_pbr_keep==1 && max_pbr_keep==1 && vertex_keep==1 && tinkeep==1) ) ; 
	...
	public static double GetRange(double[] sln, int yardage){
		double size=sln[BCOMP_MAXRANGE*10+1];
		if (yardage<size){
